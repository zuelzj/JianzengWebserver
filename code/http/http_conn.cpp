...
void http_conn::init()
{
    ...
    memset(m_real_file, '\0', FILENMAE_LEN);
}
...
字节都已分析完毕，第checked_index～(read_index-1)字节由下面的循环挨个分析*/
    ...
    for (; m_checked_idx < m_read_idx; ++m_checked_idx)
    {
        ...
        if (temp == '\r')
        {
            ...
            {
                return LINE_OPEN; // 恰好是最后一个说明没读完，需要继续读
            }
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text)
{
    ...
    // Cookie: session_id=abc123; user_pref=xyz789
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    const char *p = strrchr(m_url, '/'); // 从右向左查找出第一次出现的位置的指针
                                         // 处理cgi
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        strncpy(m_real_file + len, m_url_real, FILENMAE_LEN - len - 1);
        free(m_url_real);
        // 将用户名和密码提取出来
        // user=123&passwd=123
    ...
    else if (*(p + 1) == '7')
    {
        ...
        strncpy(m_real_file + len, m_url, FILENMAE_LEN - len - 1);
...
void http_conn::process()
{
    ...
    modfd(m_epollfd, m_sockfd, EPOLLOUT); //写完后注册写事件
}
