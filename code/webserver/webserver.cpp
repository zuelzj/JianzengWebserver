...
WebServer::WebServer()
{
    ...
    getcwd(server_path,200);//存储当前工作目录的路径
    char root[6]="/root";
...
void WebServer::eventListen()
{
    ...
    if (0 == m_OPT_LINGER)
    {
        struct linger tmp = {0, 1}; //不启用linger 丢弃未发送的数据
        setsockopt(m_listenfd, SOL_SOCKET, SO_LINGER, &tmp, sizeof(tmp));
    }
    else if (1 == m_OPT_LINGER)
    {
        struct linger tmp = {1, 1}; //启用linger 关闭链接将发送缓冲区中未发送的数据
        setsockopt(m_listenfd, SOL_SOCKET, SO_LINGER, &tmp, sizeof(tmp));
    }
    int ret=0;
    ...
    int flag = 1;//快速重启端口，timewait时允许端口绑定
    ...
    assert(ret!=-1);
    utils.setnonblocking(m_pipefd[1]);//非阻塞套接字 使信号发送不会被阻塞
    utils.addfd(m_epollfd,m_pipefd[0],false);
    utils.addsig(SIGPIPE, SIG_IGN);//当一个进程尝试向一个已关闭的管道或套接字写入触发
